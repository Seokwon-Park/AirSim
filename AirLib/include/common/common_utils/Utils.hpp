...
//TODO: Use GCC and Clang version from https://github.com/scottt/debugbreak
    ...
    static void writeBMPfile(const uint8_t* const image_data, int width, int height, const std::string& path)
    {

        FILE* f;
        unsigned char* img = NULL;
        int filesize = 54 + 3 * width * height; //width is your image width, h is image height, both int
        ...
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                int x = i;
                int y = j;
                int id = (j * width + i) * 3; // Pixel index

                img[(x + y * width) * 3 + 2] = image_data[id + 2];
                img[(x + y * width) * 3 + 1] = image_data[id + 1];
                img[(x + y * width) * 3 + 0] = image_data[id];
            }
        }
